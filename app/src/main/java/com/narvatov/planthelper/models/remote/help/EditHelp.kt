...
@Composable
fun EditHelp(
    viewModel: EditHelpViewModel = getViewModel()
) {
    val helpVal = viewModel.helpFlow.collectAsState(null)

    if (helpVal.value == null) return

    val help = helpVal.value!!

    var title by rememberSaveable { mutableStateOf(help.title) }
    ...
    LazyColumn (modifier = Modifier.fillMaxSize().padding(horizontal = 40.dp)) {
        ...
        item {
            Column {
                ...
                val locationTags = help.tags.firstOrNull { it.title == TagTitle.Location.title }?.values ?: emptyList()
                ...
                val ageGroupTags = help.tags.firstOrNull { it.title == TagTitle.AgeGroup.title }?.values ?: emptyList()
                ...
                val topicTags = help.tags.firstOrNull { it.title == TagTitle.Topic.title }?.values ?: emptyList()
